...
import newData from '../util/newsletter'
import { json, soure, schema,data2 } from "./data";
JsonEditor.setComponent("form", "el-form", ({ vm }) => {
  // vm is the JsonEditor VM

  const labelWidth = "220px";
  const model = vm.data;
  const rules = {};
  ...
  methods: {
    init() {
      ...
      for (const i in soureData) {
        ...
        // console.log(soureData[i]);
        for (const j in item){
          this.formartData = JSON.parse(item[j]);
      ...
      console.log(this.formartData,'2222')
    },
    submit(_e) {
